...
import { sendRegistrationOtp, submitRegistration } from "@/lib/api/endpoints";
...
export default function SignupVerifyScreen() {
  ...
  async function attempt(value: string) {
    ...
    setHelper(null);
    try {
      await submitRegistration({ cin: flow!.cin, otpCode: value });
      setOtpState("verified");
      setTimeout(() => router.replace("/signup-submitted"), 500);
    } catch (err) {
  ...
  async function handleResend() {
    ...
    try {
      await sendRegistrationOtp({ cin: flow!.cin, channel: flow!.channel ?? "EMAIL" });
      setCode("");
      setAttemptKey((k) => k + 1);
      setOtpState("idle");
      setHelper(null);
    ...
    <AuthScreen>
      <View className="gap-2">
        ...
        <Text className="font-sans text-[14px] text-text-secondary">
          Sent to{" "}
          <Text className="font-sans-semibold text-text-primary">{flow.maskedDestination}</Text>.
        </Text>
      </View>
