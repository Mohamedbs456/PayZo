...
// Ported from Client-Web-App/src/lib/rib.ts. UX-only mirror — the backend is
// the authority and re-validates every RIB.

export function normalizeRib(s: string): string {
  return s.replace(/\s+/g, "");
}

// Mod-97 chunked arithmetic — JS Number can't safely hold a 20-digit int.
