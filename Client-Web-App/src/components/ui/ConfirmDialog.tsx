...
const VARIANT_STYLES: Record<ConfirmVariant, VariantStyle> = {
  ...
  warning: {
    ...
    confirmBtn:
      "bg-warning text-white hover:bg-warning/90 shadow-[0_4px_12px_rgba(245,158,11,0.30)]",
  },
  positive: {
    Icon: CheckCircle2,
    iconWrapper: "bg-positive/15 text-positive",
    confirmBtn:
  ...
  primary: {
    Icon: null,
    iconWrapper: "",
    confirmBtn: "bg-accent text-accent-foreground hover:bg-accent/90",
  },
};
 ...
 * Enter confirms (when not busy). Used wherever a destructive or
 * irreversible action needs an explicit "are you sure?" beat.
 */
export function ConfirmDialog({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-center justify-center bg-scrim/40 px-4 py-6 backdrop-blur-sm"
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl bg-surface-card p-6 shadow-[0_24px_64px_-12px_rgba(15,23,42,0.30)]"
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <button
            ...
            onClick={onCancel}
            aria-label="Close"
            className="-m-1.5 flex size-7 shrink-0 items-center justify-center rounded-md text-text-faint transition-colors duration-150 ease-out hover:bg-surface-soft hover:text-text-primary"
          >
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          <button
            ...
            onClick={onCancel}
            disabled={busy}
            className="flex h-9 items-center rounded-full border border-border bg-surface-card px-4 font-sans text-[12px] font-semibold text-text-primary transition-all duration-150 ease-out hover:bg-surface-soft disabled:opacity-50"
          >
            {cancelLabel}
          </button>
          <button
