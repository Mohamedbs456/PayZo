 ...
 * signals expiry. Targets the clients realm here (backoffice realm on the
 * staff app, never mixed). Custom Error subclasses let the LoginForm tell
 * user-typed-wrong-password apart from Keycloak-is-down.
 */
...
/**
 * Resource-owner password credentials grant against the clients realm.
 ...
 * to expiring. Same error mapping shape as ropcLogin, so callers can tell
 * "session is over" (invalid_grant — token revoked/expired) apart from
 * "Keycloak is down" (5xx / network) and "developer broke the config"
...
export async function refreshTokens(refreshToken: string): Promise<RawTokenResponse> {
  ...
  // Map Keycloak error body for parity with ropcLogin. Previously this method
  // threw InvalidCredentialsError for ANY non-OK response, which mislabeled
  // 5xx outages as "session over" and silently logged users out on transient
  // Keycloak hiccups.
  const payload = (await response
    .json()
    .catch(() => null)) as KeycloakErrorBody | null;
  ...
  // 5xx, network glitches that returned a response, or anything we don't
  // recognize — treat as unreachable. The API client's refresh path catches
  // this and keeps the session intact so the next call can retry.
  throw new KeycloakUnreachableError(
    new Error(
      `Keycloak ${response.status}: ${description || errorCode || "unknown"}`,
    ),
  );
}
